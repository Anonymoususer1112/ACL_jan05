...
class Gemma3VllmInterface(ModelInterface):
    """Interface for Gemma-3 using vLLM direct API."""
    
    def __init__(self, model_id: str = "google/gemma-3-27b-it", dtype: str = "float16"):
        """
        Initialize the Gemma-3 vLLM interface.

        Args:
            model_name (str): The name of the Gemma-3 model to use.
            **kwargs: Additional keyword arguments for future extensions.
        """
        super().__init__("gemma3_vllm")
        self.model_id = model_id
        self.dtype = dtype
        self.llm = None
        self.sampling_params = SamplingParams(temperature=0.5, top_p=1.0, max_tokens=10)
        logger.info(f"Initialized Gemma3VllmInterface with model {self.model_id}")
    ...
    def _load_model(self):
        if self.llm is None:
            try:
                ...
                self.llm = LLM(model=self.model_id, dtype=torch.bfloat16, tensor_parallel_size=2, gpu_memory_utilization=0.85, max_model_len=8192)
    ...
    def call_model(self, text: str) -> str:
        """Call the Gemma-3 model with a given prompt."""
        try:
...
def call_gemma3_vllm(text: str) -> str:
    model_interface = Gemma3VllmInterface()
    return model_interface.call_model(text)

def batch_call_gemma3_vllm(texts: List[str], batch_size: int = 8) -> List[str]:
    model_interface = Gemma3VllmInterface()
