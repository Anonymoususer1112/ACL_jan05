...
class Gemma3HFModelInterface(TransformersModelInterface):

    def __init__(self, model_path: str = "google/gemma-3-27b-it"):
        super().__init__("gemma3_hf", model_path)

    def _initialize_model(self):
        try:
            logger.info(f"Loading Gemma3-HF from {self.model_path}")
            ...
            logger.info("Gemma3-HF loaded successfully.")
        except Exception as e:
            logger.error(f"Error loading Gemma3-HF: {e}")
            raise e
    ...
    def call_model(self, text: str, max_tokens: int = 512) -> str:
        try:
            ...
            logger.error(f"Error Calling Gemma3_hf: {str(e)}")
...
def call_gemma3_hf(text: str) -> str:
    model_interface = Gemma3HFModelInterface()
